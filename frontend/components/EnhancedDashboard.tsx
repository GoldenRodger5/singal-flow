...
export default function EnhancedDashboard() {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold">Enhanced Trading Dashboard</h2>
        <Button onClick={fetchData} variant="outline">Refresh Data</Button>
      </div>

      <Tabs defaultValue="holdings" className="space-y-4">
        <TabsList className="grid w-full grid-cols-4">
          <TabsTrigger value="holdings">Advanced Holdings</TabsTrigger>
          <TabsTrigger value="analytics">Performance Analytics</TabsTrigger>
          <TabsTrigger value="market">Market Pulse</TabsTrigger>
          <TabsTrigger value="watchlist">AI Watchlist</TabsTrigger>
        </TabsList>
              ...
              <Card key={holding.symbol} className="p-4">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center space-x-3">
                    ...
                    <Badge className={getSignalColor(holding.ai_signals.current_signal)}>
                      {holding.ai_signals.current_signal}
                    </Badge>
                  ...
                  <div className="text-right">
                    <div className="text-lg font-semibold">{formatCurrency(holding.market_value)}</div>
                    <div className={`text-sm ${holding.unrealized_pl >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                      {formatPercent(holding.unrealized_pnl_percent)}
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  ...
                  <div>
                    <div className="text-gray-500">Entry Price</div>
                    <div className="font-medium">{formatCurrency(holding.entry_price)}</div>
                  </div>
                  <div>
                    <div className="text-gray-500">Current Price</div>
                  ...
                  <div>
                    <div className="text-gray-500">Position Size</div>
                    <div className="font-medium">{holding.position_size_percent.toFixed(1)}%</div>
                  </div>
                </div>
                ...
                <div className="mt-4 grid grid-cols-3 gap-4 text-sm">
                  <div>
                    <div className="text-gray-500">Beta</div>
                    <div className="font-medium">{holding.risk_metrics.beta}</div>
                  </div>
                  <div>
                    <div className="text-gray-500">Volatility</div>
                    <div className="font-medium">{holding.risk_metrics.volatility}%</div>
                  </div>
                  <div>
                    <div className="text-gray-500">Sharpe Ratio</div>
                    <div className="font-medium">{holding.risk_metrics.sharpe_ratio}</div>
                  </div>
                </div>
              </Card>
            ))}
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <Card>
                ...
                <CardContent className="space-y-3">
                  <div className="flex justify-between">
                    <span>Return</span>
                    <span className="font-semibold">{formatPercent(performanceAnalytics.daily_performance.return_pct)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Sharpe Ratio</span>
                    <span className="font-semibold">{performanceAnalytics.daily_performance.sharpe_ratio}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Max Drawdown</span>
                    <span className="font-semibold text-red-600">{formatPercent(performanceAnalytics.daily_performance.max_drawdown)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Volatility</span>
                    <span className="font-semibold">{performanceAnalytics.daily_performance.volatility.toFixed(1)}%</span>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  <div className="flex justify-between">
                    <span>VaR (95%)</span>
                    <span className="font-semibold">{formatPercent(performanceAnalytics.risk_metrics.var_95)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Beta (SPY)</span>
                    <span className="font-semibold">{performanceAnalytics.risk_metrics.beta_spy}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Correlation (SPY)</span>
                    <span className="font-semibold">{performanceAnalytics.risk_metrics.correlation_spy}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Tracking Error</span>
                    <span className="font-semibold">{performanceAnalytics.risk_metrics.tracking_error.toFixed(1)}%</span>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  <div className="flex justify-between">
                    <span>Win Rate</span>
                    <span className="font-semibold">{(performanceAnalytics.trade_analytics.win_rate * 100).toFixed(1)}%</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Profit Factor</span>
                    <span className="font-semibold">{performanceAnalytics.trade_analytics.profit_factor.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Avg Win</span>
                    <span className="font-semibold text-green-600">{formatPercent(performanceAnalytics.trade_analytics.avg_win)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Avg Loss</span>
                    <span className="font-semibold text-red-600">{formatPercent(performanceAnalytics.trade_analytics.avg_loss)}</span>
                  </div>
                </CardContent>
              </Card>
            </div>
        ...
        <TabsContent value="market" className="space-y-4">
          {marketPulse && (
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <Card>
                <CardHeader>
                  <CardTitle>AI Market Assessment</CardTitle>
                </CardHeader>
                <CardContent className="space-y-3">
                  <div className="flex justify-between">
                    <span>Regime</span>
                    <Badge variant="outline">{marketPulse.ai_market_assessment.regime}</Badge>
                  </div>
                  <div className="flex justify-between">
                    <span>Confidence</span>
                    <span className="font-semibold">{(marketPulse.ai_market_assessment.confidence * 100).toFixed(0)}%</span>
                  </div>
                  <div>
                    <h4 className="font-medium mb-2">Key Drivers</h4>
                    <ul className="text-sm space-y-1">
                      {marketPulse.ai_market_assessment.key_drivers.map((driver, idx) => (
                        <li key={idx} className="text-green-600">• {driver}</li>
                      ))}
                    </ul>
                  </div>
                  <div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                    {Object.entries(marketPulse.sector_performance).map(([sector, performance]) => (
                      <div key={sector} className="text-center">
                        ...
                        <div className={`text-lg font-bold ${performance >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                          {formatPercent(performance)}
                        </div>
                      </div>
                    ))}
            ...
            {watchlistSignals.map((signal) => (
              <Card key={signal.symbol} className="p-4">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="text-right">
                    <div className="text-lg font-semibold">{formatCurrency(signal.current_price)}</div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  ...
                  <div>
                    <div className="text-gray-500">Technical</div>
                    <div className="font-medium">{(signal.technical_score * 100).toFixed(0)}%</div>
                  </div>
                  <div>
                    <div className="text-gray-500">Sentiment</div>
                    <div className="font-medium">{(signal.sentiment_score * 100).toFixed(0)}%</div>
                  </div>
                </div>
              </Card>
            ))}
