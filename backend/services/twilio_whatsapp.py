...
class WhatsAppNotifier:
    """Service for sending trading alerts via WhatsApp using Twilio."""
    ...
    async def send_trade_alert(self, recommendation: Dict[str, Any], explanation: Dict[str, Any]) -> bool:
        """Send a trade alert via WhatsApp."""
        try:
            if not self.client:
            ...
            if self.daily_alert_count >= self.max_daily_alerts:
                logger.warning("Daily WhatsApp alert limit reached")
                return False
    ...
    async def send_message(self, message: str) -> bool:
        ...
        try:
            if not self.client:
                logger.info(f"[FALLBACK] Message content:\n{message}")
                return False
    ...
    async def _send_message(self, message: str) -> bool:
        ...
        try:
            if not self.client:
                logger.info(f"[DEMO MODE] WhatsApp message:\n{message}")
                return True
            ...
            # In demo mode, still log the message
            logger.info(f"[FALLBACK] Message content:\n{message}")
            return False
    ...
    def reset_daily_counter(self) -> None:
        ...
        self.daily_alert_count = 0
        logger.info("Daily WhatsApp alert counter reset")
